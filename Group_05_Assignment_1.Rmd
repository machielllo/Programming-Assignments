...
b.vector <- c((x1 %% 2) >= 1) 
x1[b.vector]
# NEATER MAYBE:  c((as.integer(x1) %% 2) == 1)
#I'm checking the first number, how do I check the last?
#b.vector.2 <- c((x1 * 10^6 %% 2 == 0)
#nope more decimals than that.. and even then 
#uniform draws have different number of integers.

i.vector <- which(b.vector, TRUE) # i know i know
x1[i.vector] 
```
Boolean is more convenient, as we have to check a statement.
...
length(passed) #number of passed students
mean(passed)  #average of passed students
```

 c)
```{r}
boolean.index <- c(grades < 5.5)
student.numbers <- setdiff(1:n2, which(boolean.index))
max(student.numbers)
```
...
repeat {
   aux1 <- aux1 + 1
   top.ten <- append(top.ten, which(grades == max(grades2)))
...
repeat {
   aux2 <- aux2 + 1
   bot.ten <- append(bot.ten, which(grades == min(grades3)))
   grades3 <- grades3[grades3 != min(grades3)]
   if (aux2 == 10) {
      break
   }
}
top.bot <- c(top.ten, bot.ten); top.bot
grades[top.bot]
```
...
```{r}
set.seed(11031936)
big.draw <- rpois(100, 2.2)
big.draw
A <- matrix(big.draw, nrow = 10)
```
